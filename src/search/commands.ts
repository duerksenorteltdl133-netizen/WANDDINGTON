...
export function printSearchStatus(status = getPiWebAccessStatus()): void {
	...
	if (!status.configExists) {
		...
		printInfo("  waddington search set auto");
		printInfo("  waddington search set perplexity <api-key>");
		printInfo("  waddington search set exa <api-key>");
		printInfo("  waddington search set gemini <api-key>");
	}
}

export function setSearchProvider(provider: PiWebSearchProvider, apiKey?: string): void {
	if (!SEARCH_PROVIDERS.includes(provider)) {
		throw new Error(`Usage: waddington search set <${SEARCH_PROVIDERS.join("|")}> [api-key]`);
	}
	if (apiKey !== undefined && provider === "auto") {
		throw new Error("The auto provider does not use an API key. Usage: waddington search set auto");
	}
